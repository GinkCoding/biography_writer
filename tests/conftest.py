...
@pytest.fixture
def embedding_dim():
    """提供向量维度（优先读取现有数据库，否则动态计算）"""
    import sqlite3
    from pathlib import Path
    import numpy as np

    db_path = project_root / ".vector_db" / "materials.db"
    if db_path.exists():
        try:
            conn = sqlite3.connect(db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT embedding FROM materials WHERE embedding IS NOT NULL LIMIT 1")
            row = cursor.fetchone()
            conn.close()
            if row and row[0]:
    ...
    # 无历史向量时，使用轻量TF-IDF方案动态获取维度
    from src.embedding import EmbeddingManager

    manager = EmbeddingManager(
        config={"priority": ["tfidf"]},
        auto_prompt=False,
    )
    emb = manager.encode(["向量维度测试文本"])
